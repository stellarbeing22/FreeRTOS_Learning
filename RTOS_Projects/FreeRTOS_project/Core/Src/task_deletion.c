...
int main(void)
{
  ...
  status = xTaskCreate(button_task, "Button_task", 128, NULL, 3, &Button);
  configASSERT(status == pdPASS);
  (void)status;
...
static void red_led_Handler(void *parameters)
{
	...
	xLastWakeTimeRed = xTaskGetTickCount();
	while(1)
	{
		vTaskDelayUntil(&xLastWakeTimeRed,delay);
		xTaskNotifyWait(0xFFFFFFFF, 0xFFFFFFFF, &deleteTrue, 0);
...
static void green_led_Handler(void *parameters)
{
	...
	while(1)
	{
		vTaskDelayUntil(&xLastWakeTimeGreen,delay);
		xTaskNotifyWait(0xFFFFFFFF, 0xFFFFFFFF, &deleteTrue, 0);
		if(deleteTrue)
		{
			deleteTrue = 0;
...
static void yellow_led_Handler(void *parameters)
{
	...
	while(1)
	{
		...
		vTaskDelayUntil(&xLastWakeTimeYellow,delay);
		xTaskNotifyWait(0xFFFFFFFF, 0xFFFFFFFF, &deleteTrue, 0);
		if(deleteTrue)
		{
			deleteTrue = 0;
...
static void button_task(void *parameters)
{
	...
	uint8_t lastBtnState = GPIO_PIN_SET;
	TickType_t delay = pdMS_TO_TICKS(10);
	while(1)
	{
		currentBtnState = HAL_GPIO_ReadPin(B1_GPIO_Port,B1_Pin);
		if((!(currentBtnState)&&currentBtnState != lastBtnState))
		{
			switch(buttonEventCount)
			{
				case 0:
					xTaskNotify(red_led,1,eSetBits);
					buttonEventCount++;
					break;

				case 1:
					xTaskNotify(green_led,1,eSetBits);
					buttonEventCount++;
					break;

				case 2:
					xTaskNotify(yellow_led,1,eSetBits);
					buttonEventCount++;
					break;

				case 3:
					vTaskResume(red_led);
					vTaskResume(green_led);
					vTaskResume(yellow_led);
					buttonEventCount = 0;
					break;
